...
import sys
caffe_root = '/dados/caffe-nv/'  # this file is expected to be in {caffe_root}/examples
...
def classify(caffemodel, deploy_file, image_files,
    ...
    # Classify the image
    scores, positive, negative = forward_pass(images, net, transformer, batch_size=batch_size)
    '''
    ### Process the results
    ...
    for index, classification in enumerate(classifications):
        ...
        print
    '''
    return positive, negative, images
...
if __name__ == '__main__':
    ...
    positive, negative, images = classify(args['caffemodel'], args['deploy_file'], args['image_file'],
            args['mean'], args['labels'], args['batch_size'], not args['nogpu'])

    ## CODE HERE ============================================================

    result = (positive > negative).astype(np.uint8)
    contours = cv2.findContours(result,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
   	
    p = cv2.minAreaRect(contours[0][0])
	
    p1x, p1y = p[0]
    p2x, p2y = p[1]
    scale_x = 1360/42
    scale_y = 800/24
    p2x = round(scale_x * (p2x + p1x))
    p2y = round(scale_y * (p2y + p1y))
    p1x = int(round(scale_x*p1x))
    p2x = int(p2x)
    p1y = int(round(scale_x*p1y))
    p2y = int(p2y)

    im = np.zeros([800,1360])
    cv2.rectangle(images[0], (p1x, p1y), (p2x, p2y), (255,255,0),2)
    b,g,r = cv2.split(images[0])       # get b,g,r
    images[0] = cv2.merge([r,g,b])     # switch it to rgb
    cv2.imshow("min area rect", images[0])
    cv2.waitKey()
